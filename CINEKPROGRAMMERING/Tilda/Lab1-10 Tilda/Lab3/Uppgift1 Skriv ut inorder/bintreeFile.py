...
class Bintree:
    ...
    def __contains__(self,söktaVärde):
        ...
        if self.root!=None:
            if True==finns(self.root,söktaVärde):
                return True
            else:
                return False
...
## Förklaring för dubblet:
#Om initial vilkoret inte uppfylls d.v.s. att värdet är < eller >
#så innebär det att den är en dubblett och skrivs ut.
...
#Eller tills det finns inga fler nivåer (vänster/höger-Nodar) vilket isf returnerar (False)
